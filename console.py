...
class HBNBCommand(cmd.Cmd):
    ...
    def do_create(self, args):
        ...
        for n in range(len(n_args) - 1):
            try:
                update_cmd = "{} {} {} {}".format(
                    n_args[0], new_instance.id, n_args[n + 1].split('=')[0],
                    n_args[n + 1].split('=')[1].replace('_', ' '))
                self.do_update(update_cmd, new_instance)
            except IndexError:
                pass
    ...
    def check_id(self, arg):
        ...
        if key in storage.all() or os.getenv('HBNB_TYPE_STORAGE') == "db":
            return True
        print("** no instance found **")
    ...
    def do_update(self, arg, obj=None):
        ...
        if self.check_id(arg):
            if self.check_attr(arg):
                ...
                attr_exists = hasattr(obj, args_list[2])
                if attr_exists and os.getenv('HBNB_TYPE_STORAGE') != "db":
                    try:
